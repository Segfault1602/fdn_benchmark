...
{
    assert(eqCombs.size() == static_cast<size_t>(FDNorder));
    output = 0;

    // std::vector<double> intermediateOutput(FDNorder, 0);
    std::array<double, 64> intermediateOutput{}; // assuming max FDNorder is 64
...
void FDN::recalculateCoeffs(bool init)
{
    ...
    if (init)
    {
        for (auto eqComb : eqCombs)
            delete eqComb;

        eqCombs.clear();
        for (int i = 0; i < FDNorder; ++i)
        {
            eqCombs.push_back(new EQComb(dLen[i]));
        }
    }
    ...
    for (int i = 0; i < FDNorder; ++i)
    {
        aceq(i);
        for (int j = 0; j < Global::numOctaveBands + 1; ++j)
            eqCombs[i]->setFilter(j, std::vector<double>{numsOpt[0][j], numsOpt[1][j], numsOpt[2][j], densOpt[0][j],
                                                         densOpt[1][j], densOpt[2][j]});
    }
}
...
void FDN::interactionMatrix(double* G, double gw, double* wg, double* wc, double* bw)
{
    ...
    if (abs(sumG) != 0)
    {
        ...
        {                                              // Filters
            ...
            for (int k = 0; k < Global::numDesignFreqs;
                 k++) // wc is vector of central frqs, so the sized is 10 - most liekly, in this case at least
            {
                H = (complexCoeffs[0] + complexCoeffs[1] * exp(-i * wc[k]) + complexCoeffs[2] * exp(-2.0 * i * wc[k])) /
                    (complexCoeffs[3] + complexCoeffs[4] * exp(-i * wc[k]) + complexCoeffs[5] * exp(-2.0 * i * wc[k]));
...
void FDN::aceq(int idx)
{
    ...
    for (int i = 0; i < Global::numOctaveBands; i++)
        wg[i] = 2.0 * std::numbers::pi * Global::fc1[i] / fs; // Command gain frequencies in radians

    for (int j = 0; j < Global::numDesignFreqs; j++)
        wc[j] = 2.0 * std::numbers::pi * fc2[j] /
                fs; // Center frequencies in radians for iterative design with extra points
    ...
    if (Global::numOctaveBands == 10)
    {
        ...
        bw[Global::numOctaveBands - 1] =
            0.76 * wg[Global::numOctaveBands - 1]; // Additional adjustmenst due to asymmetry
    }
    ...
    double wH = 2.0 * std::numbers::pi * fH / fs;          // frequency in[rad]
...
void FDN::getDelayLines()
{
    ...
    case randomDLen:
    {
        ...
        for (int i = 0; i < FDNorder; ++i)
        {
            dLen[i] = round(minDelayLength + (maxDelayLength - minDelayLength) * dis(gen));
        }
        break;
    }
    case gaussian:
    {
        ...
        std::normal_distribution<double> distribution((maxDelayLength + minDelayLength) * 0.5,
                                                      (maxDelayLength - minDelayLength) * 0.2);
    ...
    case primes:
    {
        ...
        std::uniform_int_distribution<> dist(0, numPrimes - 1);
        for (int i = 0; i < FDNorder; ++i)
            dLen[i] = possiblePrimes[dist(gen)];
        break;
    }
    ...
    case uniform:
    {
        ...
        std::uniform_int_distribution<> dist(0, round(bandWidth));
        for (int i = 0; i < FDNorder; ++i)
        {
            dLen[i] = round(minDelayLength + i * bandWidth + dist(gen));
        }
        break;
    }
...
void FDN::setScatteringMatrix(MatrixType matType)
{
    ...
    case householder:
    {
        //            if (log(FDNorder) / log(4) == floor(log(FDNorder) / log(4))) // if the order is of the form 4^n
    ...
    case randomMat:
    {
        ...
        // std::cout << "A^T * A = I (should be identity matrix within machine precision (|non-diagonal value| <
        // 1e-15))"
        //           << std::endl;
